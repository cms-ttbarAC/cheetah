...
cwd = os.getcwd()
hpd = cwd.rstrip("cheetah")+"/asimov/python/"
if hpd not in sys.path:
    sys.path.insert(0,hpd)
hpd = cwd.rstrip("cheetah")+"/hepPlotter/python/"
if hpd not in sys.path:
    sys.path.insert(0,hpd)
...
dnn.classes = {"top":0,"antitop":1}  # target = (charge>0) ? 1:0; (from Event.cxx)
...
dnn.train(ndims=1)          # build and train the model! 'ndims' for plots comparing features
